...
public class DatabaseJobseekerPostgre {
    ...
    public static void addJobseeker(Jobseeker jobseeker) throws EmailAlreadyExistsException {
        try {
            ...
            String sql = String.format("INSERT INTO jobseeker (jobseeker_id, name, email, password, join_date)"
                    + "VALUES (%d, '%s', '%s', '%s', '%s');", getLastId()+1, jobseeker.getName(), jobseeker.getEmail(), jobseeker.getPassword(), jobseeker.getJoinDate().getTime());
    ...
    public static Jobseeker getJobseekerById(int id) throws JobSeekerNotFoundException {
        ...
        try {
            Statement stat = c.createStatement();
            String query = String.format("SELECT * FROM jobseeker WHERE jobseeker_id = %d;", id);
            ResultSet queryRes = stat.executeQuery(query);
            queryRes.next();

            int jobseeker_id = queryRes.getInt("jobseeker_id");
            String name = queryRes.getString("name");
            String email = queryRes.getString("email");
            String password = queryRes.getString("password");
            Date join_date= queryRes.getDate("join_date");
            ...
            jobseeker = new Jobseeker(jobseeker_id, name, email, password, calendar);
